// 注册一个全局自定义指令 `v-clamp`  js实现多行文本溢出...
Vue.directive('clamp', {
    // 当被绑定的元素插入到 DOM 中时……
    inserted: function (el, binding) {
        console.log(el, binding)
        $clamp(el, {clamp: binding.value.clamp});
    }
});
new Vue({
    el: '#app',
...
// MEAT AND POTATOES (MMMM, POTATOES...) ______________________________________
    ...
    function truncate(target, maxHeight) {
        ...
        if (chunks) {
            //It fits
            let elementHeight = element.clientHeight || element.offsetHeight;
...
// CONSTRUCTOR ________________________________________________________________
    ...
    if (supportsNativeClamp && opt.useNativeClamp) {
        ...
        var elementHeight = element.clientHeight || element.offsetHeight
...
function getIsClamped (element,opt) {
    let isClamped = false;
...
function getMaxLines(element, height) {
    var availHeight = height || element.clientHeight || element.offsetHeight,
